...
class BNNEnsemble(nn.Module):
    def __init__(self, input_size=3, output_size=1, hidden_layer_size=32,
                 prior_sigma_lay=0.01, epochs=1000, bias_in=True):
    ...
    def fit(self, X_feature_space, y_true):
    ...
    def predict(self, X_features, num_samples=100):
        self.eval()
        predictions = []
        with torch.no_grad():
    ...
    def save_model_weights(self, file_prefix="ENSEMBLE", directory='saved_models/pibnn'):
...
class KalmanFilterUpdater:
    def __init__(self, state_size=6):
        """
        Initialize the Kalman Filter with a constant observation model matrix H_k_model.

        Args:
            H_k_model (np.ndarray): The observation model matrix of shape (6, 6).
        """
        self.H_k_model = np.identity(state_size)  # measurement matrix [assuming already converted]

    def apply_update(self, state_estimate_minus_1, p_k_minus_1, z_k_observation_vector, sensor_sigma_vec):
        """
